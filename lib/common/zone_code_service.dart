...
class ZoneCodeService {
  ...
  Future fetchZoneCode() async {
    ...
    // 获取用户信息列表
    final jsonStr = await rootBundle.loadString('mock/world_zone_code.json');
    ...
    // print(_zoneCodeList.length);

    for (int i = 0, length = _zoneCodeList.length; i < length; i++) {
      String pinyin;
      if(SaveData.english){
        pinyin = PinyinHelper.getPinyinE(_zoneCodeList[i].en);
      }else{
        pinyin = PinyinHelper.getPinyinE(_zoneCodeList[i].name);
      }
